...
export default function UsersUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = usersRecord
      ? {
          ...
          ...usersRecord,
          preferredLocation,
          favoriteSites: linkedFavoriteSites,
        }
      : initialValues;
    ...
    setFavoriteSites(cleanValues.favoriteSites ?? []);
  ...
  const [linkedFavoriteSites, setLinkedFavoriteSites] = React.useState([]);
  const canUnlinkFavoriteSites = true;
  React.useEffect(() => {
    const queryData = async () => {
      ...
      const preferredLocationRecord = record
        ? await record.preferredLocation
        : undefined;
      setPreferredLocation(preferredLocationRecord);
      const linkedFavoriteSites = record
        ? await record.favoriteSites.toArray()
        : [];
      setLinkedFavoriteSites(linkedFavoriteSites);
    };
    queryData();
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const promises = [];
          const favoriteSitesToLink = [];
          const favoriteSitesToUnLink = [];
          const favoriteSitesSet = new Set();
          const linkedFavoriteSitesSet = new Set();
          favoriteSites.forEach((r) =>
            favoriteSitesSet.add(getIDValue.favoriteSites?.(r))
          );
          linkedFavoriteSites.forEach((r) =>
            linkedFavoriteSitesSet.add(getIDValue.favoriteSites?.(r))
          );
          linkedFavoriteSites.forEach((r) => {
            if (!favoriteSitesSet.has(getIDValue.favoriteSites?.(r))) {
              favoriteSitesToUnLink.push(r);
            }
          });
          favoriteSites.forEach((r) => {
            if (!linkedFavoriteSitesSet.has(getIDValue.favoriteSites?.(r))) {
              favoriteSitesToLink.push(r);
            }
          });
          favoriteSitesToUnLink.forEach((original) => {
            if (!canUnlinkFavoriteSites) {
              throw Error(
                `FavoriteSites ${original.id} cannot be unlinked from Users because undefined is a required field.`
              );
            }
            promises.push(
              DataStore.save(
          ...
          favoriteSitesToLink.forEach((original) => {
            promises.push(
              DataStore.save(
                FavoriteSites.copyOf(original, (updated) => {
                  updated.user = usersRecord;
                })
              )
            );
          });
          ...
          promises.push(
            DataStore.save(
              Users.copyOf(usersRecord, (updated) => {
                ...
                if (!modelFieldsToSave.preferredLocation) {
    ...
    >
      ...
      <ArrayField
        ...
        runValidationTasks={async () =>
          await runValidationTasks("favoriteSites", currentFavoriteSitesValue)
        }
