...
export default function BooksUpdateForm(props) {
  const {
    id: idProp,
    books: booksModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = booksRecord
      ? { ...initialValues, ...booksRecord }
      : initialValues;
    setBookName(cleanValues.bookName);
    setBookDescription(cleanValues.bookDescription);
    setBookImage(cleanValues.bookImage);
    setErrors({});
  };
  const [booksRecord, setBooksRecord] = React.useState(booksModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Books, idProp)
        : booksModelProp;
      setBooksRecord(record);
    };
    queryData();
  }, [idProp, booksModelProp]);
  React.useEffect(resetStateValues, [booksRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Books.copyOf(booksRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "BooksUpdateForm")}
      {...rest}
    >
      <TextField
        label="Book name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || booksModelProp)}
